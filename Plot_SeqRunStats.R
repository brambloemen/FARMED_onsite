...
KMAdata$Species_ab <- paste0(KMAdata$Species, " - ", signif(KMAdata$Perc_gDNA, digits = 2), "%")
...
readlengths <- readstats %>% ggplot(aes(x = Experiment, y = lengths)) +
  ...
  geom_text(data = N50s, aes(x = Experiment, y = N50, label = N50)) +
  ylab("Read length") +
  xlab(NULL) +
  scale_y_log10()
...
fig1 <- ggarrange(readlengths + theme(axis.text.x = element_blank()), 
                 seq_yield_plot, 
...
KMAdata2_all <- KMAdata2 %>%
  filter(!str_detect(Species, "[P|p]lasmid")) %>%
  filter(mean_queryID > 80, mean_template_coverage > 10)
